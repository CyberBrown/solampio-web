...
// Configuration - ONLY process products MISSING seo_description_summary
const DELAY_MS = 6000; // 6 seconds = 10 RPM
const CHECKPOINT_FILE = './seo-missing-checkpoint.json';
const LOG_FILE = './seo-missing.log';
...
async function main() {
  ...
  log('='.repeat(60));
  log('SEO Batch Optimizer - MISSING SUMMARIES ONLY');
  log('='.repeat(60));
  ...
  // Fetch ONLY products MISSING seo_description_summary
  log('Fetching products MISSING summaries from D1...');
  const productsJson = execSync(
    `npx wrangler d1 execute ${DB_NAME} --remote --json --command "SELECT sku, title, brand_id, item_group, price, description, cf_image_id, weight_lbs FROM storefront_products WHERE variant_of IS NULL AND is_visible = 1 AND seo_description_summary IS NULL AND sku NOT LIKE 'test%' ORDER BY sku"`,
  ...
  log(`Found ${products.length} products missing summaries`);

...
const generateProducts = (subcategoryName: string, count: number) => {
  return Array.from({ length: count }, (_, i) => ({
    id: `${subcategoryName.toLowerCase().replace(/\s+/g, '-')}-${i + 1}`,
    name: `${subcategoryName} Product ${i + 1}`,
...
export default component$(() => {
  ...
  const subcategoryName = subcategory?.name || subcategorySlug.split('-').map((w) => w.charAt(0).toUpperCase() + w.slice(1)).join(' ');
  const productCount = subcategory?.count || 0;
  const siblingSubcategories = category?.subcategories || [];

  // Generate placeholder products
  const products = generateProducts(subcategoryName, Math.min(productCount, 12));
        ...
        <div class="px-6">
          ...
          <nav class="mb-4">
            <ol class="flex items-center gap-2 text-sm flex-wrap">
              ...
              <li class="text-white/30">/</li>
              <li><Link href={`/products/category/${categorySlug}/`} class="text-white/50 hover:text-white transition-colors">{categoryName}</Link></li>
              <li class="text-white/30">/</li>
          ...
          <div class="grid md:grid-cols-2 xl:grid-cols-3 gap-4">
            {products.map((product) => (
              <div key={product.id} class="bg-white rounded-lg border border-gray-200 overflow-hidden group hover:shadow-lg transition-shadow">
                ...
                <div class="p-4">
                  <p class="text-xs font-mono text-[#c3a859] uppercase tracking-wide mb-1">{product.subcategory}</p>
        ...
        <div class="px-6">
          <div class="flex flex-col md:flex-row items-center justify-between gap-6">
            <div>
              ...
              <p class="text-white/70 mt-1">Our engineers can help you select the right {subcategoryName.toLowerCase()} for your project.</p>
            </div>
...
export const head: DocumentHead = ({ params }) => {
  ...
  const category = categories.find((c) => c.slug === categorySlug);
  const subcategory = category?.subcategories.find((s) => s.slug === subcategorySlug);
  const subcategoryName = subcategory?.name || subcategorySlug.split('-').map((w: string) => w.charAt(0).toUpperCase() + w.slice(1)).join(' ');
